  ...
  assessments: router({
    create: professorProcedure
      .input(z.object({
        ...
        plankSeconds: z.number().optional(),
      ...
      .mutation(async ({ ctx, input }) => {
        if (!ctx.user.gymId || !ctx.user.professorId) {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Dados do professor não encontrados" });
        }
        ...
        const result = await db.createPhysicalAssessment({
          studentId: input.studentId,
          professorId: ctx.user.professorId,
          gymId: ctx.user.gymId,
          assessmentDate: input.assessmentDate,
          weight: input.weight?.toString(),
          height: input.height?.toString(),
          bodyFat: input.bodyFat?.toString(),
          muscleMass: input.muscleMass?.toString(),
          ...
          tricepsSkinfold: input.tricepsSkinfold?.toString(),
          subscapularSkinfold: input.subscapularSkinfold?.toString(),
          pectoralSkinfold: input.pectoralSkinfold?.toString(),
          midaxillarySkinfold: input.midaxillarySkinfold?.toString(),
          suprailiacSkinfold: input.suprailiacSkinfold?.toString(),
          abdominalSkinfold: input.abdominalSkinfold?.toString(),
          thighSkinfold: input.thighSkinfold?.toString(),
          flexibility: input.flexibility?.toString(),
          pushups: input.pushups,
          plankSeconds: input.plankSeconds,
          vo2max: input.vo2max?.toString(),
          photoFront: input.photoFront,
          photoSide: input.photoSide,
          photoBack: input.photoBack,
          goals: input.goals,
          notes: input.notes,
          nextAssessmentDate: input.nextAssessmentDate,
        });
    ...
    getById: professorProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const assessment = await db.getAssessmentById(input.assessmentId, ctx.user.gymId);
        if (!assessment) {
          throw new TRPCError({ code: "NOT_FOUND", message: "Avaliação não encontrada" });
        }
        return assessment;
    ...
    getLatest: professorProcedure
      .input(z.object({ studentId: z.number() }))
      .query(async ({ ctx, input }) => {
        if (!ctx.user.gymId) {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Nenhuma academia associada" });
        }
        return await db.getLatestAssessment(input.studentId, ctx.user.gymId);
      }),

    getProgressData: professorProcedure
      .input(z.object({ studentId: z.number() }))
      .query(async ({ ctx, input }) => {
        if (!ctx.user.gymId) {
          throw new TRPCError({ code: "BAD_REQUEST", message: "Nenhuma academia associada" });
        }
        return await db.getAssessmentProgressData(input.studentId, ctx.user.gymId);
      }),
    ...
    update: professorProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // SECURITY: Validate that assessment belongs to professor's gym
        const assessment = await db.getPhysicalAssessmentById(input.assessmentId, ctx.user.gymId);
        if (!assessment) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Avaliação não encontrada ou não pertence à sua academia"
          });
        }

        const { assessmentId, ...updates } = input;
        const updateData: any = {};

        if (updates.weight !== undefined) updateData.weight = updates.weight.toString();
        if (updates.height !== undefined) updateData.height = updates.height.toString();
        if (updates.bodyFat !== undefined) updateData.bodyFat = updates.bodyFat.toString();
        if (updates.muscleMass !== undefined) updateData.muscleMass = updates.muscleMass.toString();
        if (updates.chest !== undefined) updateData.chest = updates.chest.toString();
        if (updates.waist !== undefined) updateData.waist = updates.waist.toString();
        if (updates.hips !== undefined) updateData.hips = updates.hips.toString();
        if (updates.rightArm !== undefined) updateData.rightArm = updates.rightArm.toString();
        if (updates.leftArm !== undefined) updateData.leftArm = updates.leftArm.toString();
        if (updates.rightThigh !== undefined) updateData.rightThigh = updates.rightThigh.toString();
        if (updates.leftThigh !== undefined) updateData.leftThigh = updates.leftThigh.toString();
        if (updates.rightCalf !== undefined) updateData.rightCalf = updates.rightCalf.toString();
        if (updates.leftCalf !== undefined) updateData.leftCalf = updates.leftCalf.toString();
        if (updates.tricepsSkinfold !== undefined) updateData.tricepsSkinfold = updates.tricepsSkinfold.toString();
        if (updates.subscapularSkinfold !== undefined) updateData.subscapularSkinfold = updates.subscapularSkinfold.toString();
        if (updates.pectoralSkinfold !== undefined) updateData.pectoralSkinfold = updates.pectoralSkinfold.toString();
        if (updates.midaxillarySkinfold !== undefined) updateData.midaxillarySkinfold = updates.midaxillarySkinfold.toString();
        if (updates.suprailiacSkinfold !== undefined) updateData.suprailiacSkinfold = updates.suprailiacSkinfold.toString();
        if (updates.abdominalSkinfold !== undefined) updateData.abdominalSkinfold = updates.abdominalSkinfold.toString();
        if (updates.thighSkinfold !== undefined) updateData.thighSkinfold = updates.thighSkinfold.toString();
        if (updates.flexibility !== undefined) updateData.flexibility = updates.flexibility.toString();
        if (updates.pushups !== undefined) updateData.pushups = updates.pushups;
        if (updates.plankSeconds !== undefined) updateData.plankSeconds = updates.plankSeconds;
        if (updates.vo2max !== undefined) updateData.vo2max = updates.vo2max.toString();
        if (updates.photoFront !== undefined) updateData.photoFront = updates.photoFront;
        if (updates.photoSide !== undefined) updateData.photoSide = updates.photoSide;
        if (updates.photoBack !== undefined) updateData.photoBack = updates.photoBack;
        if (updates.goals !== undefined) updateData.goals = updates.goals;
        if (updates.notes !== undefined) updateData.notes = updates.notes;
        if (updates.nextAssessmentDate !== undefined) updateData.nextAssessmentDate = updates.nextAssessmentDate;

        return await db.updatePhysicalAssessment(assessmentId, ctx.user.gymId, updateData);
      }),
    ...
    delete: professorProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        return await db.deletePhysicalAssessment(input.assessmentId, ctx.user.gymId);
      }),
  }),
