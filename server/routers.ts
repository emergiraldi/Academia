...
// Helper: calculate late fee/interest synchronously using preloaded settings (no DB queries)
function calculateLateFeeSync(payment: any, settings: any) {
  if (!settings || payment.status !== 'pending') {
    return { lateFeeInCents: 0, interestInCents: 0, totalAmountInCents: payment.amountInCents, daysOverdue: 0 };
  }
  const now = Date.now();
  ...
  return { lateFeeInCents, interestInCents, totalAmountInCents: originalAmount + lateFeeInCents + interestInCents, daysOverdue };
}
...
export const appRouter = router({
  ...
  payments: router({
    myPayments: studentProcedure.query(async ({ ctx }) => {
      ...
      // Get payments and settings in parallel
      const [payments, gymSettings] = await Promise.all([
        db.getPaymentsByStudent(student.id, ctx.user.gymId),
        db.getGymSettings(ctx.user.gymId!),
      ]);

      // Calculate late fees synchronously (no extra DB calls)
      const paymentsWithCalculations = payments.map((payment) => {
        if (payment.status === 'pending') {
          const calculated = calculateLateFeeSync(payment, gymSettings);
          return {
            ...payment,
    ...
    list: gymAdminProcedure.query(async ({ ctx }) => {
      ...
      // Fetch gym settings ONCE for all calculations
      const gymSettings = await db.getGymSettings(ctx.user.gymId);
    ...
    listAll: gymAdminProcedure
      ...
      .query(async ({ ctx }) => {
        ...
        // Fetch all data in parallel (single query each)
        const [payments, students, subscriptions, plans, gymSettings] = await Promise.all([
          db.listPayments(ctx.user.gymId),
          db.listStudents(ctx.user.gymId),
          db.listSubscriptions(ctx.user.gymId),
          db.listPlans(ctx.user.gymId),
          db.getGymSettings(ctx.user.gymId),
        ]);
        ...
        // Calculate late fees (sync, no DB calls) and join with other data
        const paymentsWithCalculations = payments.map((payment) => {
          let calculatedPayment = payment;

          // Calculate late fees for pending payments (using preloaded settings)
          if (payment.status === 'pending') {
            const calculated = calculateLateFeeSync(payment, gymSettings);
            calculatedPayment = {
              ...payment,
    ...
    getByStudent: gymAdminProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        // Get all payments for this student and gym settings in parallel
        const [payments, gymSettings] = await Promise.all([
          db.getPaymentsByStudent(input.studentId, ctx.user.gymId),
          db.getGymSettings(ctx.user.gymId!),
        ]);
  ...
  staff: router({
    ...
    uploadFaceImage: gymAdminProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        // Upload to Control ID
        console.log('[uploadFaceImage-Staff] 🚀 Iniciando upload para Control ID...');
        ...
        try {
          ...
          if (controlIdService) {
            let controlIdUserId = staffMember.controlIdUserId;
            ...
            console.log('[uploadFaceImage-Staff] 📊 Resultado do upload:', JSON.stringify(result, null, 2));

            if (result.success) {
              console.log('[uploadFaceImage-Staff] ✅ Marcando como cadastrado no banco...');
          ...
          console.error('[uploadFaceImage-Staff] ❌ Erro:', error);
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Erro ao enviar foto: " + error.message
          });
    ...
    create: gymAdminProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        await db.createBankAccount({
          ...
          pixCertificadoPath: input.pixCertificado || null,
          pixChavePrivadaPath: input.pixChavePrivada || null,
  ...
  gymSettings: router({
    ...
    update: gymAdminProcedure
      .input(z.object({
        gymSlug: z.string(),
        daysToBlockAfterDue: z.number().min(1).max(90).optional(),
        blockOnExpiredExam: z.boolean().optional(),
        examValidityDays: z.number().min(30).max(365).optional(),
        minimumAge: z.number().min(0).max(100).optional(),
        daysToStartInterest: z.number().min(0).max(90).optional(),
        interestRatePerMonth: z.preprocess((val) => (val === undefined || val === null || Number.isNaN(val)) ? undefined : val, z.number().min(0).max(100).optional()),
        lateFeePercentage: z.preprocess((val) => (val === undefined || val === null || Number.isNaN(val)) ? undefined : val, z.number().min(0).max(100).optional()),
        allowInstallments: z.boolean().optional(),
        maxInstallments: z.number().min(1).max(24).optional(),
        minimumInstallmentValue: z.number().min(1000).optional(),
        // SMTP Settings
