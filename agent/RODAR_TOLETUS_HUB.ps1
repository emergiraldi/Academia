...
$hubPath = Join-Path $scriptPath "hub-main\src\Toletus.Hub.API"
...
if (-not (Test-Path $hubPath)) {
    ...
    Write-Host "  C:\SysFit\agent\" -ForegroundColor Cyan
    Write-Host "  ├── hub-main\" -ForegroundColor Cyan
    Write-Host "  │   └── src\" -ForegroundColor Cyan
    Write-Host "  │       └── Toletus.Hub.API\" -ForegroundColor Cyan
