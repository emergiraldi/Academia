...
# -*- coding: utf-8 -*-
"""
Identifica pedidos com problema (agregação em Python)
"""
...
print("="*100)
print("IDENTIFICANDO PEDIDOS COM VALORES INCORRETOS (V3)")
print("="*100)
...
try:
    ...
    print(f"  {len(todos_itens):,} itens encontrados")

    # Agregar em Python
    print("\n>> Agregando valores por pedido...")
    soma_por_pedido = {}
    ...
    print(f"  {len(soma_por_pedido):,} pedidos com itens")
    ...
    for codigo, vlr_produtos, vlr_total in pedidos:
        ...
        if codigo in soma_por_pedido:
            soma_itens = soma_por_pedido[codigo]
    ...
    if pedidos_problema:
        ...
        with open(r'c:\Projeto\Academia\pedidos-com-problema.txt', 'w') as f:
            f.write(f"PEDIDOS COM DIFERENÇA ENTRE VLR_PRODUTOS E SOMA DOS ITENS\n")
            f.write(f"Total: {len(pedidos_problema)}\n\n")
            f.write(f"CODIGO\tVLR_TOTAL\tVLR_PRODUTOS\tSOMA_ITENS\tDIFERENCA\n")
            for ped in pedidos_problema:
                f.write(f"{ped['codigo']}\t{ped['vlr_total']:.2f}\t{ped['vlr_produtos']:.2f}\t{ped['soma_itens']:.2f}\t{ped['diferenca']:.2f}\n")
